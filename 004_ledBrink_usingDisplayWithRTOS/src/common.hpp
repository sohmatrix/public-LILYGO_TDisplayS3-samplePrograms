 ...
 * @file common.hpp
 * @author sohmatrix
 * @brief Common constants and settings for the program
 * @branch develop
 * @updated 2026-09-19
 */
// === SOURCE-HEADER:END === 2b8a0d498f9812046d0c175a0586284f9bdafa6fc5629964ef20289b0732d74f
...
namespace Constants_Common
{
    ...
    inline constexpr const size_t MESSAGE_COUNTER_MAX = 1000; // Maximum value for messageCounter before it resets
    inline constexpr const size_t PROCESS_TIME_COUNTER_MAX = 1000; // Maximum value for process time counter before it resets
    // Serial communication settings
    inline constexpr const unsigned long SERIAL_BAUD_RATE = 115200; // Baud rate for serial communication
};
...
namespace Constants_DelayTimes
{
    ...
    inline constexpr const uint32_t MAIN_LOOP = 2000; // Delay time in milliseconds for the loop
    ...
    // Display message task loop delay time in milliseconds for the display loop
    ...
    // LED device delay times in milliseconds for various operations
    inline constexpr const uint32_t LED_MUTEX = 100; // Delay time in milliseconds for LED mutex acquisition
    inline constexpr const  uint32_t LED_MUTEX_RETRY = 100; // Delay time in milliseconds for LED mutex retry

    // Message counter task loop delay time in milliseconds for the message counter loop
    inline constexpr const uint32_t MESSAGE_COUNTER_LOOP = 1000; // Delay time in milliseconds for the message counter loop
...
namespace Global_Variables
{
    ...
    inline void* serialLoggerInstanceHolder = nullptr; // serial instance for serial communication handling
    inline void* timeInfoItemsInstanceHolder = nullptr; // time info items instance for time info item handling
    ...
    inline void* messageCounterTaskInstanceHolder = nullptr; // message counter task instance for handling message counter tasks
    inline void* externalLedInstanceHolder = nullptr; // external LED instance for external LED handling
};

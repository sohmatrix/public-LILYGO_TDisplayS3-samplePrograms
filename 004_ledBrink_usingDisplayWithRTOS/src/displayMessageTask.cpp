...
void DisplayMessageTask::setup()
{
    ...
    if (!breakLoopFlag)
    {
        ...
        if (timeInfoItem)
        {
            ...
            timeInfoItem->updateLastTime(); // Update the last time for the display setup
        }
        else
        {
...
void DisplayMessageTask::loop()
{
    ...
    if (!breakLoopFlag)
    {
        ...
        if (timeInfoItem)
        {
            ...
            timeInfoItem->updateLastTime();          // Update the last update time for the display message task
        }
        else
        {
            //-> time info item is not available

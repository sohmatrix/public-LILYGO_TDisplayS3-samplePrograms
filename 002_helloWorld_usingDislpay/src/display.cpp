 ...
 * @file display.cpp
 * @author sohmatrix
 * @brief Display handling implementation for the program
 * @branch develop
 * @updated 2026-09-24
 */
// === SOURCE-HEADER:END === fa1ee76f28f984547b7cbcd7e5763e13df1490294628c28730537d1cf1329682
...
bool Display::initialize()
{
    ...
    delay(delayTimes::DISPLAY_INITIALIZE); // wait for a short time before returning
...
{
    return (busHandleHolder && displayHandleHolder); // Check if both busInstance and displayInstance are not nullptr
}

bool Display::displayClear()
{
    // namespace alias for global variables
    namespace globalVars = Global_Variables;
    namespace delayTimes = Constants_DelayTimes; // Use the namespace for delay times
...
bool Display::displayTitle(const char* system, const char* process)
{
    ...
    namespace delayTimes = Constants_DelayTimes; // Use the namespace for delay times
    namespace globalVars = Global_Variables; // Use the namespace for global variables
    // Retrieve the instances of the serial logger, message counter, and display from the global variables
...
bool Display::displayMessage(const char* message)
{
    ...
    if (!(isInitialized()))
    {
        ...
        // Send the error message to the serial monitor
        serialLogger->sendErrorMessage(ErrorMessageId::DISPLAY_INITIALIZATION_FAILS);
